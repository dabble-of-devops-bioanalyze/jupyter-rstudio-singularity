#!/usr/bin/env python

"""The setup script."""

from setuptools import setup, find_packages

with open("README.md") as readme_file:
    readme = readme_file.read()

with open("HISTORY.md") as history_file:
    history = history_file.read()

requirements = ["Click>=7.0", "typer[all]", "prefect>2", "prefect-shell", "colorlog"]

test_requirements = []

setup(
    author="Jillian Rowe",
    author_email="jillian@dabbleofdevops.com",
    python_requires=">=3.6",
    classifiers=[
        "Development Status :: 2 - Pre-Alpha",
        "Intended Audience :: Developers",
        "License :: OSI Approved :: Apache Software License",
        "Natural Language :: English",
        "Programming Language :: Python :: 3",
        "Programming Language :: Python :: 3.6",
        "Programming Language :: Python :: 3.7",
        "Programming Language :: Python :: 3.8",
    ],
    description="Launch RStudio with Singularity from the command line.",
    entry_points={
        "console_scripts": [
            "launch-rstudio=jupyter_rstudio_singularity.cli:cli",
        ],
    },
    install_requires=requirements,
    license="Apache Software License 2.0",
    long_description=readme + "\n\n" + history,
    include_package_data=True,
    keywords="jupyter_rstudio_singularity",
    name="jupyter_rstudio_singularity",
    packages=find_packages(
        include=["jupyter_rstudio_singularity", "jupyter_rstudio_singularity.*"]
    ),
    test_suite="tests",
    tests_require=test_requirements,
    url="https://github.com/dabble-of-devops-bioanalyze/jupyter_rstudio_singularity",
    version="0.1.0",
    zip_safe=False,
)
